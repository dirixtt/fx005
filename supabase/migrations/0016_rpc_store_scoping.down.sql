...
-- Same overload trap in reverse: drop the store-scoped signatures before
-- recreating the originals, or Postgres keeps both.
drop function checkout_order(uuid, jsonb, text, text, text, text);
drop function create_telegram_order(uuid, uuid, integer, text, text, text, bigint);
drop function get_order_status(uuid, uuid);
drop function recent_orders_by_phone(uuid, text, integer);
drop function unanswered_chats(uuid, integer);
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    select * into v_product from products where id = v_variant.product_id and is_active and show_on_storefront;
...
begin
  ...
  values ('pos', 'completed', p_customer_id, 0, 0, p_payment_method) returning id into v_sale_id;
...
as $$
  ...
  from sales s
  where s.id = p_order_id and s.channel = 'online';
$$;

create or replace function recent_orders_by_phone(p_phone text, p_limit integer default 3)
...
create or replace function unanswered_chats(p_minutes integer default 15)
